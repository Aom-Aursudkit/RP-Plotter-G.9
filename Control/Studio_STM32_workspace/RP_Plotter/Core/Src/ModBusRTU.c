...
// Speed and acceleration: normal conversion
	REG16(REG_SPEED_R) = (int16_t) (RAD_TO_DEG(AngularVelocity_rad) * 10.0f);
	REG16(REG_ACCELERATION_R) = (int16_t) (RAD_TO_DEG(AngularAcceleration_rad)
			* 10.0f);

	REG16(REG_SPEED_THETA) = (int16_t) Velocity_mm * 10.0f;
	REG16(REG_ACCELERATION_THETA) = (int16_t) Acceleration_mm * 10.0f;
